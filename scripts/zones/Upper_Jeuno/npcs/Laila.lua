...
function onTrigger(player,npc)
    local lakesideMin = player:getQuestStatus(JEUNO,xi.quest.id.jeuno.LAKESIDE_MINUET);
    ...
    elseif ((player:getQuestStatus(JEUNO,xi.quest.id.jeuno.THE_UNFINISHED_WALTZ) == QUEST_AVAILABLE
        or (player:getQuestStatus(JEUNO,xi.quest.id.jeuno.THE_UNFINISHED_WALTZ) == QUEST_COMPLETED
        and player:hasItem(19203) == false))
        and player:getMainJob() == xi.job.DNC and player:getMainLvl()>=40) then

        player:startEvent(10129);
    elseif (player:getQuestStatus(JEUNO,xi.quest.id.jeuno.THE_UNFINISHED_WALTZ) == QUEST_ACCEPTED and player:getCharVar("QuestStatus_DNC_AF1") == 5 and player:seenKeyItem(xi.ki.THE_ESSENCE_OF_DANCE) and player:getMainJob() == xi.job.DNC) then
        player:startEvent(10133);
    elseif (player:getQuestStatus(JEUNO,xi.quest.id.jeuno.THE_UNFINISHED_WALTZ) == QUEST_ACCEPTED) then
        player:startEvent(10134);

    -- Dancer AF: The Road to Divadom
    elseif (player:getQuestStatus(JEUNO,xi.quest.id.jeuno.THE_UNFINISHED_WALTZ) == QUEST_COMPLETED
        and player:getQuestStatus(JEUNO,xi.quest.id.jeuno.THE_ROAD_TO_DIVADOM) == QUEST_AVAILABLE
    ...
    elseif (player:getQuestStatus(JEUNO,xi.quest.id.jeuno.THE_ROAD_TO_DIVADOM) == QUEST_COMPLETED
        and player:getQuestStatus(JEUNO, xi.quest.id.jeuno.COMEBACK_QUEEN) == QUEST_AVAILABLE
        and player:getMainJob() == xi.job.DNC) then
...
function onEventFinish(player,csid,option)
    if (csid == 10111 and option == 1) then
        player:addQuest(JEUNO,xi.quest.id.jeuno.LAKESIDE_MINUET);
        player:addKeyItem(xi.ki.STARDUST_PEBBLE)
    elseif (csid == 10118) then
        player:setCharVar("Lakeside_Minuet_Progress",0);
        player:completeQuest(JEUNO,xi.quest.id.jeuno.LAKESIDE_MINUET);
    ...
    elseif (csid== 10129) then
        if (player:getQuestStatus(JEUNO,xi.quest.id.jeuno.THE_UNFINISHED_WALTZ) == QUEST_COMPLETED) then
            player:delQuest(JEUNO,xi.quest.id.jeuno.THE_UNFINISHED_WALTZ);
            player:delKeyItem(xi.ki.THE_ESSENCE_OF_DANCE);
        end
        player:addQuest(JEUNO,xi.quest.id.jeuno.THE_UNFINISHED_WALTZ)
    ...
    elseif (csid== 10133) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(JEUNO,xi.quest.id.jeuno.THE_UNFINISHED_WALTZ);
        end

    -- Dancer AF: The Road to Divadom
    elseif (csid == 10136) then -- Road To Divadom pt 1
        player:setCharVar("roadToDivadomCS", 1);
        player:addQuest(JEUNO, xi.quest.id.jeuno.THE_ROAD_TO_DIVADOM);
    ...
    elseif (csid == 10170) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(JEUNO, xi.quest.id.jeuno.THE_ROAD_TO_DIVADOM)
    ...
    elseif (csid == 10143) then
        ...
        player:addQuest(JEUNO, xi.quest.id.jeuno.COMEBACK_QUEEN);
    ...
    elseif (csid == 10211) then -- finally reward the player
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(JEUNO, xi.quest.id.jeuno.COMEBACK_QUEEN);
