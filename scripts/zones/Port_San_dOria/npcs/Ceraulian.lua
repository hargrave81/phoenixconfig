...
function onEventFinish(player,csid,option)
    ...
    if (csid == 24) then
        player:addTitle(tpz.title.HEIR_TO_THE_HOLY_CREST);
        ...
        player:completeQuest(SANDORIA,tpz.quest.id.sandoria.THE_HOLY_CREST);
        player:setPetName(tpz.pet.type.WYVERN,1);
    -- Chasing Quotas (DRG AF2)
    elseif (csid == 18) then
        if option == 0 then
