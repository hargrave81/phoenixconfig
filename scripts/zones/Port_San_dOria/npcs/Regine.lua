...
function onTrade(player,npc,trade)
    local flyersForRegine = player:getQuestStatus(SANDORIA,xi.quest.id.sandoria.FLYERS_FOR_REGINE);
    local theBrugaireConsortium = player:getQuestStatus(SANDORIA,xi.quest.id.sandoria.THE_BRUGAIRE_CONSORTIUM);
...
function onTrigger(player,npc)
    ...
    -- FLYERS FOR REGINE
    if (player:getQuestStatus(SANDORIA,xi.quest.id.sandoria.FLYERS_FOR_REGINE) == QUEST_AVAILABLE and ffr == 0) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 510 and option == 2) then
        if (npcUtil.giveItem(player, {{532,12}, {532,3}} )) then
            player:addQuest(SANDORIA,xi.quest.id.sandoria.FLYERS_FOR_REGINE);
            player:setCharVar("FFR",17);
        end
    elseif (csid == 603) then
        if (npcUtil.completeQuest(player, SANDORIA, xi.quest.id.sandoria.FLYERS_FOR_REGINE, {gil=440, title=xi.title.ADVERTISING_EXECUTIVE})) then
    ...
    elseif (csid == 510 and option == 0) then
        local stockA =
        {
            4641,1165,2, -- Scroll of Diaga
            4664,837,2,  -- Scroll of Slow
            4662,7025,2, -- Scroll of Stoneskin
