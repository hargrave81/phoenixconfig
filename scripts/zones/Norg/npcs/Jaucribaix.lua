...
function onTrade(player,npc,trade)
    ...
    elseif (player:getQuestStatus(OUTLANDS, xi.quest.id.outlands.FORGE_YOUR_DESTINY) == QUEST_ACCEPTED and npcUtil.tradeHas(trade, {1152, 1153})) then -- Bomb Steel, Sacred Branch
        player:startEvent(27)
    elseif (player:getQuestStatus(OUTLANDS, xi.quest.id.outlands.THE_SACRED_KATANA) == QUEST_ACCEPTED and player:hasKeyItem(xi.ki.HANDFUL_OF_CRYSTAL_SCALES) and npcUtil.tradeHas(trade, 17809)) then -- Mumeito
        player:startEvent(141)
    elseif (player:getQuestStatus(OUTLANDS, xi.quest.id.outlands.A_THIEF_IN_NORG) == QUEST_ACCEPTED and player:hasKeyItem(xi.ki.CHARRED_HELM) and npcUtil.tradeHas(trade, 823)) then -- Gold Thread
        player:startEvent(162)
    end
end

function onTrigger(player,npc)
    local forgeYourDestiny  = player:getQuestStatus(OUTLANDS, xi.quest.id.outlands.FORGE_YOUR_DESTINY)
    local theSacredKatana   = player:getQuestStatus(OUTLANDS, xi.quest.id.outlands.THE_SACRED_KATANA)
    local yomiOkuri         = player:getQuestStatus(OUTLANDS, xi.quest.id.outlands.YOMI_OKURI)
    local aThiefinNorg      = player:getQuestStatus(OUTLANDS, xi.quest.id.outlands.A_THIEF_IN_NORG)
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 25 and option == 1) then
        player:addQuest(OUTLANDS, xi.quest.id.outlands.FORGE_YOUR_DESTINY)
        player:addItem(1152)
        player:addItem(1153)        
        player:messageSpecial(ID.text.ITEM_OBTAINED, 1152);
        player:messageSpecial(ID.text.ITEM_OBTAINED, 1153);
    elseif (csid == 27) then
        player:confirmTrade()
        player:setCharVar("ForgeYourDestiny_timer", os.time() + 68) -- 3 game days
    ...
    elseif (csid == 139 and option == 1) then
        player:addQuest(OUTLANDS, xi.quest.id.outlands.THE_SACRED_KATANA)
    ...
    elseif (csid == 146 and option == 1) then
        player:addQuest(OUTLANDS,xi.quest.id.outlands.YOMI_OKURI)
    ...
    elseif (csid == 158 and option == 1) then
        player:addQuest(OUTLANDS,xi.quest.id.outlands.A_THIEF_IN_NORG)
