...
function onTrigger(player, npc)
    if player:hasStatusEffect(dsp.effect.REFRESH) == false and player:hasStatusEffect(dsp.effect.ATTACK_BOOST) == false then
        ...
        player:addStatusEffect(dsp.effect.EVASION_BOOST, 30, 0, 1200)
        player:addStatusEffect(dsp.effect.ACCURACY_BOOST, 25, 0, 1200)        
        player:addStatusEffect(dsp.effect.DEX_BOOST, 15, 0, 1200)
        player:addStatusEffect(dsp.effect.STR_BOOST, 15, 0, 1200)
        player:addStatusEffect(dsp.effect.INT_BOOST, 8, 0, 1200)
        player:addStatusEffect(dsp.effect.MAGIC_ACC_BOOST_II, 12, 0, 1200)        
        player:addStatusEffect(dsp.effect.DEFENSE_BOOST, 40, 0, 1200)
        if player:hasStatusEffect(dsp.effect.PROTECT) == false then
            player:addStatusEffect(dsp.effect.PROTECT, 130, 0, 1200)
        end
