...
function onZoneIn(player,prevZone)
    ...
    elseif (player:getCurrentMission(ACP) == xi.mission.id.acp.THOSE_WHO_LURK_IN_SHADOWS_I) then
        cs = 29;
    elseif (prevZone == xi.zone.QUBIA_ARENA and player:getQuestStatus(BASTOK,xi.quest.id.bastok.THE_FIRST_MEETING) == QUEST_ACCEPTED and not player:hasKeyItem(xi.ki.LETTER_FROM_DALZAKK)) then
        cs = 16; -- MNK AF
    elseif (prevZone == xi.zone.BEAUCEDINE_GLACIER and player:getQuestStatus(SANDORIA,xi.quest.id.sandoria.PIEUJE_S_DECISION) == QUEST_ACCEPTED and player:getCharVar("pieujesDecisionCS") == 0) then
        cs = 19; -- WHM AF
    end
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 29) then
        player:completeMission(ACP,xi.mission.id.acp.THOSE_WHO_LURK_IN_SHADOWS_I);
        player:addMission(ACP,xi.mission.id.acp.THOSE_WHO_LURK_IN_SHADOWS_II);
    end
end;