...
function onTrade(player,npc,trade)
    local ALittleKnowledge = player:getQuestStatus(CRYSTAL_WAR, xi.quest.id.crystalWar.A_LITTLE_KNOWLEDGE);
...
function onTrigger(player,npc)
    ...
    local onSabbatical = player:getQuestStatus(CRYSTAL_WAR,xi.quest.id.crystalWar.ON_SABBATICAL);
    local onSabbaticalProgress = player:getCharVar("OnSabbatical");
    local downwardHelix = player:getQuestStatus(CRYSTAL_WAR, xi.quest.id.crystalWar.DOWNWARD_HELIX);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 10 and option == 0) then
        player:addQuest(CRYSTAL_WAR, xi.quest.id.crystalWar.A_LITTLE_KNOWLEDGE);
        player:setCharVar("ALittleKnowledge", 1);
        player:startEvent(12, 1);
    elseif (csid == 12) then
        player:tradeComplete();
        player:setCharVar("ALittleKnowledge", 2);
        player:messageSpecial("Activate Manafont, Chainspell, Astral Flow, or Azure Lore");
    elseif (csid == 14) then
        ...
        player:completeQuest(CRYSTAL_WAR, xi.quest.id.crystalWar.A_LITTLE_KNOWLEDGE);
    elseif (csid == 47) then
    ...
    elseif (csid == 18) then
        player:addQuest(CRYSTAL_WAR, xi.quest.id.crystalWar.ON_SABBATICAL);
    ...
    elseif (csid == 20) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(CRYSTAL_WAR,xi.quest.id.crystalWar.ON_SABBATICAL);
    ...
    elseif (csid == 23) then
        ...
        player:addQuest(CRYSTAL_WAR, xi.quest.id.crystalWar.DOWNWARD_HELIX);
    elseif (csid == 25) then
    ...
    elseif (csid == 27) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(CRYSTAL_WAR, xi.quest.id.crystalWar.DOWNWARD_HELIX);
