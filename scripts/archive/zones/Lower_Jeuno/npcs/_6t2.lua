...
function onTrigger(player,npc)
    ...
    local ANewDawn = player:getQuestStatus(JEUNO,xi.quest.id.jeuno.A_NEW_DAWN);
    local ANewDawnEvent = player:getCharVar("ANewDawn_Event");
    local ScatteredIntoShadow = player:getQuestStatus(JEUNO,xi.quest.id.jeuno.SCATTERED_INTO_SHADOW);

    local SaveMySon = player:getCharVar("SaveMySon_Event");

    local ChocobosWounds = player:getQuestStatus(JEUNO, xi.quest.id.jeuno.CHOCOBO_S_WOUNDS);
    ...
    -- Save My Son
    elseif (player:getQuestStatus(JEUNO, xi.quest.id.jeuno.SAVE_MY_SON) == QUEST_AVAILABLE and mLvl >= ADVANCED_JOB_LEVEL) then
        player:startEvent(164);
    elseif (player:getQuestStatus(JEUNO, xi.quest.id.jeuno.SAVE_MY_SON) == QUEST_ACCEPTED) then
        if (SaveMySon == 0) then
            player:startEvent(229);
        elseif (SaveMySon == 1) then
            player:startEvent(163);
        end
    elseif (player:needToZone() == false and player:getQuestStatus(JEUNO, xi.quest.id.jeuno.SAVE_MY_SON) == QUEST_COMPLETED and SaveMySon == 2) then
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 164 and option == 0) then  -- start save my son
        player:addQuest(JEUNO, xi.quest.id.jeuno.SAVE_MY_SON);
        player:setCharVar("SaveMySon_Event",1);
    elseif (csid == 163) then -- complete Save My Son
        if (player:getFreeSlotsCount(0) >= 1) then
            ...
            player:completeQuest(JEUNO,xi.quest.id.jeuno.SAVE_MY_SON);
        else
    ...
    elseif (csid == 5) then
        ...
        if (option == 1) then
            player:addQuest(JEUNO, xi.quest.id.jeuno.A_NEW_DAWN);
