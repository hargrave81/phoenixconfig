...
function doHealingBreath(player, threshold, breath)
    ...
    if player:getHPP() < threshold and inBreathRange(player) then
        ...
        for _,member in ipairs(party) do
            if member:getHPP() < threshold and inBreathRange(player) then
...
function onMobSpawn(mob)
    ...
    pet:addStatusEffect(tpz.effect.DEF_BOOST,foodPower*1.5,0,9600,0,0)
    mob:addStatusEffect(tpz.effect.STR_BOOST,foodPower,0,9600,0,0)
    mob:addStatusEffect(tpz.effect.INT_BOOST,foodPower,0,9600,0,0)
