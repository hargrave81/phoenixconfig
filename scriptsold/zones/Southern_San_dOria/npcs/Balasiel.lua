...
function onTrade(player,npc,trade)
    ...
    elseif (player:getQuestStatus(SANDORIA,xi.quest.id.sandoria.A_SQUIRE_S_TEST) == QUEST_ACCEPTED) then
...
function onTrigger(player,npc)
    ...
    local aSquiresTest = player:getQuestStatus(SANDORIA, xi.quest.id.sandoria.A_SQUIRE_S_TEST)
    local aSquiresTestII = player:getQuestStatus(SANDORIA,xi.quest.id.sandoria.A_SQUIRE_S_TEST_II)
    local aKnightsTest = player:getQuestStatus(SANDORIA, xi.quest.id.sandoria.A_KNIGHT_S_TEST)

    if (wsQuestEvent ~= nil) then
        player:startEvent(wsQuestEvent)
    elseif (player:getQuestStatus(SANDORIA,xi.quest.id.sandoria.KNIGHT_STALKER) == QUEST_ACCEPTED and player:getCharVar("KnightStalker_Progress") == 2) then
...
function onEventFinish(player,csid,option)
    if (csid == 616) then
        if (option == 0) then
            player:addQuest(SANDORIA,xi.quest.id.sandoria.A_SQUIRE_S_TEST)
        else
    ...
    elseif (csid == 617) then
        if (player:getFreeSlotsCount(0) >= 1) then
            ...
            player:completeQuest(SANDORIA,xi.quest.id.sandoria.A_SQUIRE_S_TEST)
        else
            player:messageSpecial(ID.text.ITEM_CANNOT_BE_OBTAINED, 16565) -- Spatha
        end
    elseif (csid == 625 or csid == 630) then
        player:addQuest(SANDORIA,xi.quest.id.sandoria.A_SQUIRE_S_TEST_II)
    elseif (csid == 626) then
        ...
        player:addFame(SANDORIA,30)
        player:completeQuest(SANDORIA,xi.quest.id.sandoria.A_SQUIRE_S_TEST_II)
    elseif (csid == 627) then
        if (option == 0) then
            player:addQuest(SANDORIA,xi.quest.id.sandoria.A_KNIGHT_S_TEST)
    ...
    elseif (csid == 628) then
        if (player:getFreeSlotsCount(0) >= 1) then
            ...
            player:addFame(SANDORIA,30)
            player:completeQuest(SANDORIA,xi.quest.id.sandoria.A_KNIGHT_S_TEST)
        else
