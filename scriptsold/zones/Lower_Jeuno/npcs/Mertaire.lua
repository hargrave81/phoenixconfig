...
function onTrade(player,npc,trade)
    local theOldMonument = player:getQuestStatus(JEUNO,xi.quest.id.jeuno.THE_OLD_MONUMENT)
    local aMinstrelInDespair = player:getQuestStatus(JEUNO,xi.quest.id.jeuno.A_MINSTREL_IN_DESPAIR)
...
function onTrigger(player,npc)
    local theOldMonument = player:getQuestStatus(JEUNO,xi.quest.id.jeuno.THE_OLD_MONUMENT)
    local painfulMemory = player:getQuestStatus(JEUNO,xi.quest.id.jeuno.PAINFUL_MEMORY)
    local theRequiem = player:getQuestStatus(JEUNO,xi.quest.id.jeuno.THE_REQUIEM)
    local circleOfTime = player:getQuestStatus(JEUNO,xi.quest.id.jeuno.THE_CIRCLE_OF_TIME)
    ...
    if theOldMonument == QUEST_AVAILABLE and level >= ADVANCED_JOB_LEVEL then
        ...
        player:completeQuest(JEUNO,xi.quest.id.jeuno.PATH_OF_THE_BARD);
...
function onEventFinish(player,csid,option)
    ...
    elseif csid == 101 then
        ...
        player:completeQuest(JEUNO,xi.quest.id.jeuno.A_MINSTREL_IN_DESPAIR)
        player:addFame(JEUNO, 30)
    ...
    elseif (csid == 137 or csid == 138) and option == 1 then
        player:addQuest(JEUNO,xi.quest.id.jeuno.PAINFUL_MEMORY)
    ...
    elseif csid == 139 then
        player:addQuest(JEUNO,xi.quest.id.jeuno.THE_CIRCLE_OF_TIME)
