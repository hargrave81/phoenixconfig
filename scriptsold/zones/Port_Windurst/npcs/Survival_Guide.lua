...
function onTrigger(player,npc)
    if player:getCharVar("WindyMissionFast") == 1 then
        ...
        local areaMissionIds = xi.mission.id[xi.mission.area[2]]
        local missionId = 0
        ...
        if not player:hasKeyItem(xi.keyItem.STARWAY_STAIRWAY_BAUBLE) then
            player:addKeyItem(xi.keyItem.STARWAY_STAIRWAY_BAUBLE)
        end
        ...
        if not player:hasKeyItem(xi.keyItem.MESSAGE_TO_JEUNO_WINDURST) then
            player:addKeyItem(xi.keyItem.MESSAGE_TO_JEUNO_WINDURST)
        end
