...
local function suppliesAvailableBitmask(player, nation)
    ...
    if mask ~= -1 and mask ~= 4294967295 then
        for i = 0, 18 do
            if GetRegionOwner(i) ~= nation or i == 16 or i == 17 or (i == 18 and not player:hasCompletedMission(COP, xi.mission.id.cop.DARKNESS_NAMED)) then
                mask = mask + 2^(i + 5)
            end
        end
    end
...
xi.conquest.teleporterOnTrigger = function(player, teleporterNation, teleporterEvent)
    if player:getNation() == teleporterNation then
        ...
        local teleporterRegion = xi.region.SANDORIA + teleporterNation
        ...
        local a6 =
        {
            [xi.nation.SANDORIA] = 1,
            [xi.nation.BASTOK]   = 256,
            [xi.nation.WINDURST] = 512,
        }
...
xi.conquest.teleporterOnEventFinish = function(player, csid, option, teleporterEvent)
    if csid == teleporterEvent then
        ...
        elseif option >= 1029 and option <= 1047 then
            ...
            if xi.conquest.canTeleportToOutpost(player, region) and player:getCP() >= fee then
                player:delCP(fee)
                player:addStatusEffectEx(xi.effect.TELEPORT, 0, xi.teleport.id.OUTPOST, 0, 1, 0, region)
            end
        end
