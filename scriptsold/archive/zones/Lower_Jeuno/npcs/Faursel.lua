...
function onTrade(player,npc,trade)
    ...
    local questStatus = player:getQuestStatus(JEUNO,xi.quest.id.jeuno.THE_ROAD_TO_AHT_URHGAN);
    local questStatusVar = player:getCharVar("THE_ROAD_TO_AHT_URHGAN");

    if (questStatus == QUEST_ACCEPTED and questStatusVar == 1) then

        if (trade:hasItemQty(537,1) == true or trade:hasItemQty(538,1) == true or trade:hasItemQty(539,1) == true
        or trade:hasItemQty(540,1) == true or trade:hasItemQty(541,1) == true or trade:hasItemQty(542,1) == true and trade:getItemCount() >= 1 and trade:getGil() == 0) then -- Beginner List (Any Subjob Items)
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 10062 and option == 1) then -- Offer Quest, First Dialog.
        player:addQuest(JEUNO,xi.quest.id.jeuno.THE_ROAD_TO_AHT_URHGAN);
    ...
    elseif (csid == 10068) then
        player:completeQuest(JEUNO,xi.quest.id.jeuno.THE_ROAD_TO_AHT_URHGAN);
