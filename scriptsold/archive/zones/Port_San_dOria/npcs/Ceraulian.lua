...
function onTrade(player,npc,trade)
    if (player:getQuestStatus(SANDORIA,xi.quest.id.sandoria.CHASING_QUOTAS) == QUEST_ACCEPTED and player:getCharVar("ChasingQuotas_Progress") == 0 and
...
function onTrigger(player,npc)
    local Quotas_Status = player:getQuestStatus(SANDORIA,xi.quest.id.sandoria.CHASING_QUOTAS);
    local Quotas_Progress = player:getCharVar("ChasingQuotas_Progress");
    local Quotas_No = player:getCharVar("ChasingQuotas_No");
    local Stalker_Status = player:getQuestStatus(SANDORIA,xi.quest.id.sandoria.KNIGHT_STALKER);
    local Stalker_Progress = player:getCharVar("KnightStalker_Progress");

    if (player:getMainLvl() >= ADVANCED_JOB_LEVEL and player:getQuestStatus(SANDORIA,xi.quest.id.sandoria.THE_HOLY_CREST) == QUEST_AVAILABLE) then
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 24) then
        player:addTitle(xi.title.HEIR_TO_THE_HOLY_CREST);
        player:unlockJob(xi.job.DRG);
        player:messageSpecial(ID.text.YOU_CAN_NOW_BECOME_A_DRAGOON);
        player:setCharVar("TheHolyCrest_Event",0);
        player:addFame(SANDORIA,30);
        player:completeQuest(SANDORIA,xi.quest.id.sandoria.THE_HOLY_CREST);
        player:setPetName(xi.pet.type.WYVERN,1);
    -- Chasing Quotas (DRG AF2)
    elseif (csid == 18) then
        if option == 0 then
            player:setCharVar("ChasingQuotas_No",1);
        else
            player:addQuest(SANDORIA,xi.quest.id.sandoria.CHASING_QUOTAS);
        end
    ...
    elseif (csid == 15) then
        if (player:getFreeSlotsCount() < 1) then
            ...
            player:completeQuest(SANDORIA,xi.quest.id.sandoria.CHASING_QUOTAS);
