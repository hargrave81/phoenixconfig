...
function onTrigger(player,npc)
    ...
    local theFangedOne = player:getQuestStatus(WINDURST,xi.quest.id.windurst.THE_FANGED_ONE) -- RNG flag quest
    local theFangedOneCS = player:getCharVar("TheFangedOne_Event")
    local sinHunting = player:getQuestStatus(WINDURST,xi.quest.id.windurst.SIN_HUNTING)-- RNG AF1
    local sinHuntingCS = player:getCharVar("sinHunting")
    local fireAndBrimstone = player:getQuestStatus(WINDURST,xi.quest.id.windurst.FIRE_AND_BRIMSTONE)-- RNG AF2
    local fireAndBrimstoneCS = player:getCharVar("fireAndBrimstone")
    local unbridledPassion = player:getQuestStatus(WINDURST,xi.quest.id.windurst.UNBRIDLED_PASSION)-- RNG AF3
...
function onEventFinish(player,csid,option)
    ...
    elseif csid == 351 then
        player:addQuest(WINDURST, xi.quest.id.windurst.THE_FANGED_ONE)
    ...
    elseif csid == 523 then -- start quest RNG AF1
        player:addQuest(WINDURST, xi.quest.id.windurst.SIN_HUNTING)
    ...
    elseif csid == 531 then -- start RNG AF2
        player:addQuest(WINDURST, xi.quest.id.windurst.FIRE_AND_BRIMSTONE)
    ...
    elseif csid == 541 then -- start RNG AF3
        player:addQuest(WINDURST, xi.quest.id.windurst.UNBRIDLED_PASSION)
