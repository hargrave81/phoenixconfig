...
uint8 calcSynthResult(CCharEntity* PChar)
{
    ...
    if(result != SYNTHESIS_FAIL) // It has gone through the cycle without breaking
    {
        switch(finalhqtier)
        {
            case 4:  chance = 0.650;    break; // 1 in 2
            case 3:  chance = 0.350;    break; // 1 in 4
            case 2:  chance = 0.150;    break; // 1 in 16
            case 1:  chance = 0.045;    break; // 1 in 64
        ...
        {
            random = tpzrand::GetRandomNumber(0, 24);

            if (random <= 0 + hqtier)
                result = SYNTHESIS_HQ3;
            else if (random < 6 + hqtier * 2)
